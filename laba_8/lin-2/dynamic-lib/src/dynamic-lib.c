...
int OpenMapping(const char *filePath, int listSizeDef)
{
    ...
    if (fileStat.st_size == 0 && listSize == 0)
    {
    ...
    else if (fileStat.st_size == 0 && listSize != 0) 
    {
        listSize = listSizeDef; // По умолчанию размер списка
        size_t fileSize = sizeof(struct Student) * (listSize);
        if (ftruncate(fd, fileSize) == -1)
        {
            perror("Failed to set file size");
